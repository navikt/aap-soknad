...
export const getBedriftSchema = (getText: Function) => [
  ...
  yup.object().shape({
    // Hvilken type støtte
    typeStoette: yup.string().ensure().required(getText('form.typeStoette.required')),
  }),
  yup.object().shape({
    // Personlig info
    kommune: string().required(paakrevdMelding('form.personlig.kommune', getText)),
    tlfPrivat: string()
            ...
            return number()
              ...
              .when('fraAar', (fraAar, schema) => {
                if (fraAar) {
                  return schema.min(
                    fraAar,
                    (tilAar: { value: number }) => `${fraAar} er etter ${tilAar.value}`
                  );
                } else {
                  return undefined;
                }
  ...
  object().shape({
    praksis: yup.array().of(
      object().shape({
        ...
        fraDato: yup.date().required(paakrevdMelding('form.praksis.fraDato', getText)),
    ...
    etablererstipend: object().shape({
      ...
      resultat: yup
        ...
        .when('soektOm', {
          ...
          then: yup
            .string()
            .oneOf(['avslaatt', 'innvilget', 'ikkeFerdigBehandlet'])
            .nullable()
            .required(getText('form.etablererstipend.resultat.paakrevd')),
        }),
      vedlegg: yup
        .string()
        .nullable()
